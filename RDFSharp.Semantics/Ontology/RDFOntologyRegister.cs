...
namespace RDFSharp.Semantics
{
    ...
    public sealed class RDFOntologyRegister : IEnumerable<RDFOntology> {
        ...
        #region Methods
        /// <summary>
        /// Adds the given ontology to the register (only if it has unique prefix)
        /// </summary>
        ...
        public static RDFOntology GetOntologyByPrefix(String prefix) {
            if (prefix         != null && prefix.Trim() != String.Empty) {
                var regPrefixes = GetRegisteredPrefixes();
                if (regPrefixes.Contains(prefix.Trim().ToUpperInvariant())) {
